...
function videoConfirm(callback){
    ...
    dialogue( message.add(cancel), "Нужна помощь?", function() { callback(false); } );
}
                ...
                if( data.length )
                {
                    for( var i in data )
                    {
                        html +=
                            ...
                            '<a href="'+data[i]['alias']+'/ru'+'" title="'+data[i]['title']+'">'+
                            ...
                            '<a href="' + data[i]['alias'] +'/ru'+ '" title="' + data[i]['title'] + '" class="btn grey">подробнее</a>' +
                            '<a data-group_id="' + data[i]['group_id'] + '" href="#" title="" class="btn green buy">купить</a>' +
                            '</div>'+
                            '<div class="one_item_compare">' +
                            '<input type="checkbox" id="'+block_class+'_compare_item_'+data[i]['id']+'" value="'+data[i]['catalog']+'-'+data[i]['id']+'" '+(data[i]['checked'] ? 'checked="checked"' : '')+' />'+
                            '<label for="'+block_class+'_compare_item_'+data[i]['id']+'"><span></span>к сравнению</label>'+
                            '</div>'+
                            '</div>'+
                            '</div>';
                    }
            ...
            success: function (data)
            {
                ...
                for( var i in data )
                {
                    ...
                    html +=
                        ...
                        '<div class="one_item_buttons">'+
                        '<a href="' + data[i]['alias'] +'/ru'+ '" title="" class="btn grey">подробности</a>' +
                        '<a data-group_id="' + data[i]['group_id'] + '" href="javascript:;" title="" class="' + (data[i]['count_available'] != 0 ? 'btn green buy' : 'not_available grey') + '">купить</a>' +
                        '</div>'+
                        '<div class="one_item_compare">'+
    ...
    $.ajax({
        ...
        success: function (data)
        {
            ...
            else
            {
                ...
                var newCount = count + parseInt(count_items);

                $count_cart.text(newCount);
            }
            if(callback)
                callback();
        },
...
$(document).ready(function()
{
    ...
    function map_initialize( map_element_id )
    {
        ...
        var contentString1 =
            ...
            '<div class="contact_callback_phones">'+
                //'<a href="/callback" class="callback" title="Зворотній зв\'язок" id="ajax_simple_map" data-options="width: 940, height: 400" target="/callback" data-type="ajax">Зворотній зв\'язок</a>'+
            '</div>'+
            '</div>';
    ...
    if( window.location.pathname=='/contacts/ru' )
    {
        function map_initialize1( map_element_id )
        {
            ...
            var mapLat2 = 50.463991;
            var mapLng2 = 30.635547;
            ...
            var contentString1 =
                '<div id="content_map">'+
                '<div class="content_map_title">Центральный офис</div>'+
                '<p>г. Киев ул.Садова 95</p>'+
                '<p>(дачный массив Осокорки)</p>'+
                '<a href="mailto:#" class="callback email" title="написать нам">написать нам</a>'+
                ...
                '</div>'+
                '<div class="contact_callback_phones">'+
                '<a href="#" class="callback" title="">обратная связь</a>'+
                '</div>'+
                '</div>';

            var contentString2 =
                '<div id="content_map2">'+
                '<div class="content_map_title">Оптовый Склад</div>'+
                '<p>ул.Киото, 10 (Торговый павильон "Професійне насіння" №25 возле ст.метро "Лесная").</p>'+
...
function Counter (time, id, lang_id) {
    ...
    $('.days .flip-clock-label').html('Дней');
    $('.hours .flip-clock-label').html('Часов');
    $('.minutes .flip-clock-label').html('Минут');
